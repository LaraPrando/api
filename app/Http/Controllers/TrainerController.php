...
class TrainerController extends Controller
{
    ...
    {
        $validatedData = $request->validate(["name"=>"required|string|max:255", "age"=>"required|string|max:2", "height"=>"required|string|max:255", "weight"=>"required|string|max:255", "cpf"=>"required|string|max:255", "rg"=>"required|string|max:255",]);
    ...
    public function destroy(string $id)
    {
        ...
        if(!$trainer){
            return response()->json(["message"=> "traier not found"],404);
    }
    $trainer->delete();
